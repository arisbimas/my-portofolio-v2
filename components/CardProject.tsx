...
const BuilderBadge = (stacks: Array<string>) => {
  ...
  return stacks?.map((data: string, index: number) => {
    ...
    return (
      <span
        key={`stack-${index}`}
        className={`whitespace-nowrap rounded-sm px-2 py-0.5 mr-1 text-xs border-2 border-black shadow-[3px_3px_0px_0_rgba(0,0,0,1)] text-black bg-brand-${colorArray?.[colorIndex]?.name}`}
      >
        {data}
      </span>
    );
